...
# 生成四四组合
four_combinations = list(combinations(attributes, 4))

# 处理每个数据集
for df, file_name in [(train_df, 'Train_text_sum_four_combinations.csv'), 
                      (test_df, 'Test_text_sum_four_combinations.csv')]:
    # 为每个组合生成新列
    for combo in four_combinations:
        attr1, attr2, attr3, attr4 = combo
        col_name = f"{attr1}_{attr2}_{attr3}_{attr4}"
        df[col_name] = df.apply(
            lambda row: f"The headline {row[f'{attr1}_value']}, {row[f'{attr2}_value']}, {row[f'{attr3}_value']}, {row[f'{attr4}_value']}"
            if pd.notnull(row[f'{attr1}_value']) and pd.notnull(row[f'{attr2}_value']) and 
               pd.notnull(row[f'{attr3}_value']) and pd.notnull(row[f'{attr4}_value'])
            else None, axis=1)
    ...
    print(f"Saved {file_name} with four combinations")