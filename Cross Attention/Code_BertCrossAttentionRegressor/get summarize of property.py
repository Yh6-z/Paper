...
def main():
    ...
    # 保存结果到 CSV
    output_path = os.path.join(output_dir, "Attention_Linear_accuracy_results.csv")
    ...
    plt.bar(x+0.175, results_df['Accuracy'], bar_width, color='skyblue', label='Model Accuracy')
    
    
    for i, acc in enumerate(results_df['Accuracy']):
        plt.text(i+0.175 , acc , f'{acc:.4f}', ha='center', va='bottom', fontsize=10, color='black')
