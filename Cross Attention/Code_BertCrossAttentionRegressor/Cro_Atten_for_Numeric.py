...
class CTRDataset(Dataset):
    def __init__(self, df, tokenizer, max_length=128):
        ...
        self.numeric_labels = df['Numeric_Label'].astype(str).tolist()  
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'input_ids_numeric': encoding_numeric['input_ids'].squeeze(),
            'attention_mask_numeric': encoding_numeric['attention_mask'].squeeze(),
            'ctr': torch.tensor(ctr, dtype=torch.float32)
        }

# Cross Attention 模型（处理 Numeric_Label）
class BertCrossAttentionRegressor(nn.Module):
    def __init__(self, model_name="bert-base-uncased", hidden_size=768, num_attention_heads=12, dropout_prob=0.1):
        ...
        self.cross_attention_numeric = nn.MultiheadAttention(hidden_size, num_attention_heads, dropout=dropout_prob)
    ...
    def forward(self, input_ids_headline, attention_mask_headline, 
                input_ids_numeric, attention_mask_numeric, labels=None):
        # 编码标题
        ...
        cross_attn_numeric, _ = self.cross_attention_numeric(hidden_headline, hidden_numeric, hidden_numeric)
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{epochs}") as pbar:
            for batch in pbar:
                ...
                input_ids_numeric = batch['input_ids_numeric'].to(device)
                attention_mask_numeric = batch['attention_mask_numeric'].to(device)
...
def main():
    ...
    # 数据集保存路径
    output_dir = os.path.join(script_dir, "Numeric RESULT")
    os.makedirs(output_dir, exist_ok=True)  
    train_save_path = os.path.join(output_dir, f"Train_Cro_Atten_numeric_pred_seed.csv")
    test_save_path = os.path.join(output_dir, f"Test_Cro_Atten_numeric_pred_seed.csv")
    
    # 模型保存路径
    model_save_path = os.path.join(script_dir, "Numeric RESULT", f"best_model_for_numeric_seed.pth")
...
### 准确率计算：正确 1455/3263，准确率 0.446
### 准确率计算：正确 1434/3263，准确率 0.439