...
class CTRDataset(Dataset):
    def __init__(self, df, tokenizer, max_length=128):
        ...
        self.concreteness_labels = df['Concreteness_Label'].astype(str).tolist()  
    ...
    def __getitem__(self, idx):
        ...
        concreteness_label = self.concreteness_labels[idx]
        ctr = self.ctrs[idx]
        ...
        # 编码 Concreteness_Label
        encoding_concreteness = self.tokenizer(
            concreteness_label,
            max_length=self.max_length,
            padding='max_length',
            truncation=True,
        ...
        return {
            ...
            'input_ids_concreteness': encoding_concreteness['input_ids'].squeeze(),
            'attention_mask_concreteness': encoding_concreteness['attention_mask'].squeeze(),
...
class BertCrossAttentionRegressor(nn.Module):
    def __init__(self, model_name="bert-base-uncased", hidden_size=768, num_attention_heads=12, dropout_prob=0.1):
        ...
        self.cross_attention_concreteness = nn.MultiheadAttention(hidden_size, num_attention_heads, dropout=dropout_prob)
        ...
        print(f"Cross Attention Concreteness heads: {self.cross_attention_concreteness.num_heads}")
    
    def forward(self, input_ids_headline, attention_mask_headline, 
                input_ids_concreteness, attention_mask_concreteness, labels=None):
        # 编码标题
        ...
        # 编码 Concreteness_Label
        outputs_concreteness = self.bert(input_ids_concreteness, attention_mask=attention_mask_concreteness)
        hidden_concreteness = outputs_concreteness.last_hidden_state
        
        # 转置为 [seq_len, batch_size, hidden_size]
        hidden_headline = hidden_headline.transpose(0, 1)
        hidden_concreteness = hidden_concreteness.transpose(0, 1)
        
        # Cross Attention（Concreteness_Label）
        cross_attn_concreteness, _ = self.cross_attention_concreteness(hidden_headline, hidden_concreteness, hidden_concreteness)
        cross_attn_concreteness = cross_attn_concreteness.transpose(0, 1)
        
        # 融合向量
        attn_output = cross_attn_concreteness
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{epochs}") as pbar:
            for batch in pbar:
                ...
                input_ids_concreteness = batch['input_ids_concreteness'].to(device)
                attention_mask_concreteness = batch['attention_mask_concreteness'].to(device)
                labels = batch['ctr'].to(device)
                
                outputs = model(
                    input_ids_headline, attention_mask_headline,
                    input_ids_concreteness, attention_mask_concreteness,
                    labels
                )
                loss = outputs['loss']
                loss.backward()
...
def main():
    ...
    print(script_dir)
    parent_dir = os.path.dirname(script_dir)  # 返回当前目录的父目录的上一级
    ...
    # 数据集保存路径
    output_dir = os.path.join(script_dir, "Concreteness RESULT")
    os.makedirs(output_dir, exist_ok=True)  
    train_save_path = os.path.join(output_dir, f"Train_Cro_Atten_concreteness_pred_seed.csv")
    test_save_path = os.path.join(output_dir, f"Test_Cro_Atten_concreteness_pred_seed.csv")
    
    
    # 模型保存路径
    model_save_path = os.path.join(script_dir,"Concreteness RESULT",f"best_model_for_concreteness_Seed.pth")
    try:
        ...
        # 验证 Concreteness_Label 列是否存在
        if 'Concreteness_Label' not in train_df.columns or 'Concreteness_Label' not in test_df.columns:
            print("错误：数据集中缺少 'Concreteness_Label' 列")
            exit(1)
        # 验证删除结果
