...
class CTRDataset(Dataset):
    def __init__(self, df, tokenizer, max_length=128):
        ...
        self.pronoun_labels = df['Pronoun_Label'].astype(str).tolist()  # 为 Pronoun_Label
    ...
    def __getitem__(self, idx):
        ...
        pronoun_label = self.pronoun_labels[idx]
        ctr = self.ctrs[idx]
        ...
        # 编码 Pronoun_Label
        encoding_pronoun = self.tokenizer(
            pronoun_label,
            max_length=self.max_length,
            padding='max_length',
            truncation=True,
        ...
        return {
            ...
            'input_ids_pronoun': encoding_pronoun['input_ids'].squeeze(),
            'attention_mask_pronoun': encoding_pronoun['attention_mask'].squeeze(),
            'ctr': torch.tensor(ctr, dtype=torch.float32)
        }

# Cross Attention 模型（处理 Pronoun_Label）
class BertCrossAttentionRegressor(nn.Module):
    def __init__(self, model_name="bert-base-uncased", hidden_size=768, num_attention_heads=12, dropout_prob=0.1):
        ...
        self.cross_attention_pronoun = nn.MultiheadAttention(hidden_size, num_attention_heads, dropout=dropout_prob)
    ...
    def forward(self, input_ids_headline, attention_mask_headline, 
                input_ids_pronoun, attention_mask_pronoun, labels=None):
        # 编码标题
        ...
        # 编码 Pronoun_Label
        outputs_pronoun = self.bert(input_ids_pronoun, attention_mask=attention_mask_pronoun)
        hidden_pronoun = outputs_pronoun.last_hidden_state
        
        # 转置为 [seq_len, batch_size, hidden_size]
        hidden_headline = hidden_headline.transpose(0, 1)
        hidden_pronoun = hidden_pronoun.transpose(0, 1)
        
        # Cross Attention（Pronoun_Label）
        cross_attn_pronoun, _ = self.cross_attention_pronoun(hidden_headline, hidden_pronoun, hidden_pronoun)
        cross_attn_pronoun = cross_attn_pronoun.transpose(0, 1)
        
        # 融合向量
        attn_output = cross_attn_pronoun
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{epochs}") as pbar:
            for batch in pbar:
                ...
                input_ids_pronoun = batch['input_ids_pronoun'].to(device)
                attention_mask_pronoun = batch['attention_mask_pronoun'].to(device)
                labels = batch['ctr'].to(device)
                
                outputs = model(
                    input_ids_headline, attention_mask_headline,
                    input_ids_pronoun, attention_mask_pronoun,
                    labels
                )
                loss = outputs['loss']
                loss.backward()
...
def main():
    ...
    # 数据集保存路径
    output_dir = os.path.join(script_dir, "Pronoun RESULT")
    os.makedirs(output_dir, exist_ok=True)  
    train_save_path = os.path.join(output_dir, f"Train_Cro_Atten_pronoun_pred_seed.csv")
    test_save_path = os.path.join(output_dir, f"Test_Cro_Atten_pronoun_pred_seed.csv")

    # 模型保存路径
    model_save_path = os.path.join(script_dir, "Pronoun RESULT", f"best_model_for_pronoun_seed.pth")
    ...
    try:
        ...
        # 验证 Pronoun_Label 列是否存在
        if 'Pronoun_Label' not in train_df.columns or 'Pronoun_Label' not in test_df.columns:
            print("错误：数据集中缺少 'Pronoun_Label' 列")
            exit(1)
        # 验证删除结果
