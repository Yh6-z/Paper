...
class CTRDataset(Dataset):
    def __init__(self, df, tokenizer, max_length=128):
        ...
        self.lengths = df['length'].astype(str).tolist()
        self.ctrs = df['CTR'].astype(float).values
    
    def __len__(self):
        return len(self.df)
    
    def __getitem__(self, idx):     ### 调用样本数据，并及进行编码
        ...
        # 编码 length
        encoding_length = self.tokenizer(
            length,
            max_length=self.max_length,
            padding='max_length',
            truncation=True,
        ...
        return {
            ...
            'input_ids_length': encoding_length['input_ids'].squeeze(),
            'attention_mask_length': encoding_length['attention_mask'].squeeze(),
...
class BertCrossAttentionRegressor(nn.Module):
    def __init__(self, model_name="bert-base-uncased", hidden_size=768, num_attention_heads=12, dropout_prob=0.1):
        ...
        self.cross_attention_length = nn.MultiheadAttention(hidden_size, num_attention_heads, dropout=dropout_prob)
    ...
    def forward(self, input_ids_headline, attention_mask_headline, 
                input_ids_length, attention_mask_length, labels=None):
        # 编码标题
        ...
        # 编码 length
        outputs_length = self.bert(input_ids_length, attention_mask=attention_mask_length)
        hidden_length = outputs_length.last_hidden_state
        
        # 转置为 [seq_len, batch_size, hidden_size]
        hidden_headline = hidden_headline.transpose(0, 1)
        hidden_length = hidden_length.transpose(0, 1)
        
        # Cross Attention（length）
        cross_attn_length, _ = self.cross_attention_length(hidden_headline, hidden_length, hidden_length)
        cross_attn_length = cross_attn_length.transpose(0, 1)
        
        # 融合向量
        attn_output = cross_attn_length
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{epochs}") as pbar:
            for batch in pbar:
                ...
                input_ids_length = batch['input_ids_length'].to(device)
                attention_mask_length = batch['attention_mask_length'].to(device)
                labels = batch['ctr'].to(device)
                
                outputs = model(
                    input_ids_headline, attention_mask_headline,
                    input_ids_length, attention_mask_length,
                    labels
                )
                loss = outputs['loss']
                loss.backward()
...
def main():
    ...
    train_path = os.path.join(parent_dir, "Data process", "SENTIMENT_LENGTH label", "Merge RESULT", "Train_updated.csv")
    test_path = os.path.join(parent_dir, "Data process", "SENTIMENT_LENGTH label", "Merge RESULT", "Test_updated.csv")
    

     # 数据集保存路径
    output_dir = os.path.join(script_dir, "Length RESULT")
    os.makedirs(output_dir, exist_ok=True)  
    train_save_path = os.path.join(output_dir, f"Train_Cro_Atten_length_pred_seed.csv")
    test_save_path = os.path.join(output_dir, f"Test_Cro_Atten_length_pred_seed.csv")
    # 模型保存路径
    model_save_path = os.path.join(script_dir, "Length RESULT", f"best_model_for_Length_seed.pth")
    ...
    try:
        ...
        # 验证 length 列是否存在
        if 'length' not in train_df.columns or 'length' not in test_df.columns:
            print("错误：数据集中缺少 'length' 列")
            exit(1)
...
if __name__ == "__main__":
    main()
### 准确率计算：正确 1447/3263，准确率 0.444
### 准确率计算：正确 1447/3263，准确率 0.443