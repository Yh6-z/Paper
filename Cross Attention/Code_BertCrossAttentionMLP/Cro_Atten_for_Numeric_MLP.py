...
class CTRDataset(Dataset):
    def __init__(self, df, tokenizer, max_length=128):
        ...
        self.numeric_labels = df['Numeric_Label'].astype(str).tolist()  # 替换为 Numeric_Label
    ...
    def __getitem__(self, idx):
        ...
        numeric_label = self.numeric_labels[idx]  # 替换为 numeric_label
        ctr = self.ctrs[idx]
        ...
        encoding_numeric = self.tokenizer(
            numeric_label,
            max_length=self.max_length,
            padding='max_length',
            truncation=True,
        ...
        return {
            ...
            'input_ids_numeric': encoding_numeric['input_ids'].squeeze(),  # 替换为 input_ids_numeric
            'attention_mask_numeric': encoding_numeric['attention_mask'].squeeze(),  # 替换为 attention_mask_numeric
...
class BertCrossAttentionRegressor(nn.Module):
    def __init__(self, model_name="bert-base-uncased", hidden_size=768, num_attention_heads=12, dropout_prob=0.1):
        ...
        self.cross_attention_numeric = nn.MultiheadAttention(hidden_size, num_attention_heads, dropout=dropout_prob)  # 替换为 cross_attention_numeric
    ...
    def forward(self, input_ids_headline, attention_mask_headline, 
                input_ids_numeric, attention_mask_numeric, labels=None):  # 替换为 input_ids_numeric 和 attention_mask_numeric
        # 编码标题
        ...
        # 编码 Numeric_Label
        outputs_numeric = self.bert(input_ids_numeric, attention_mask=attention_mask_numeric)
        hidden_numeric = outputs_numeric.last_hidden_state
        
        # 转置为 [seq_len, batch_size, hidden_size]
        hidden_headline = hidden_headline.transpose(0, 1)
        hidden_numeric = hidden_numeric.transpose(0, 1)
        
        # Cross Attention（Numeric_Label）
        cross_attn_numeric, _ = self.cross_attention_numeric(hidden_headline, hidden_numeric, hidden_numeric)  # 替换为 cross_attention_numeric
        cross_attn_numeric = cross_attn_numeric.transpose(0, 1)
        
        # 融合向量
        attn_output = cross_attn_numeric
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{epochs}") as pbar:
            for batch in pbar:
                ...
                input_ids_numeric = batch['input_ids_numeric'].to(device)  # 替换为 input_ids_numeric
                attention_mask_numeric = batch['attention_mask_numeric'].to(device)  # 替换为 attention_mask_numeric
                labels = batch['ctr'].to(device)
                
                outputs = model(
                    input_ids_headline, attention_mask_headline,
                    input_ids_numeric, attention_mask_numeric,
                    labels
                )
                loss = outputs['loss']
                loss.backward()
...
def main():
    ...
    # 数据集保存路径
    output_dir = os.path.join(script_dir, "Numeric RESULT")  # 更新为 Numeric RESULT
    os.makedirs(output_dir, exist_ok=True)
    train_save_path = os.path.join(output_dir, f"Train_Cro_Atten_numeric_pred_seed.csv")  # 更新为 numeric
    test_save_path = os.path.join(output_dir, f"Test_Cro_Atten_numeric_pred_seed.csv")  # 更新为 numeric
    
    # 模型保存路径
    model_save_path = os.path.join(output_dir, f"best_model_for_numeric_Seed.pth")  # 更新为 numeric
    ...
    try:
        ...
        # 验证 Numeric_Label 列是否存在
        if 'Numeric_Label' not in train_df.columns or 'Numeric_Label' not in test_df.columns:
            print("错误：数据集中缺少 'Numeric_Label' 列")
            exit(1)
