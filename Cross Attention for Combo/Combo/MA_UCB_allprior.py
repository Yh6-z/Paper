...
def simulate_bandit(df, T, n_value, z_value, ini=200, output_dir='./results'):
    ...
    results = []
    selected_titles = []
    pred_columns = [f'pred_CTR_{i}' for i in range(1, 64)]  # 63个pred_CTR
    num_priors = len(pred_columns)  # 63
    prior_types = [f'pred_{i}' for i in range(1, num_priors + 1)]  # ['pred_1', 'pred_2', ..., 'pred_63']
    ...
    try:
        ...
        for test_id, group in sampled_df.groupby('test_id'):
            if len(group) == 0:
                ...
                continue
            trials_1 = np.ones(len(group))  # per臂
            trials_2 = np.ones((len(group), num_priors))  # per臂 per先验
            successes = np.zeros(len(group))  # per臂
            pred_ctrs_raw = np.array(group[pred_columns].values)
            pred_ctrs = np.maximum(pred_ctrs_raw, 0)  # 如果<0则设为0
            n_llm = ini * np.ones((len(group), num_priors))  # per臂 per先验
            ...
            for t in range(total_T):
                ...
                for k in range(num_priors):
                    ucbs[:, k] = get_UCB(trials_1, trials_2[:, k], successes, z_value, t + 1, pred_ctrs[:, k], n_llm[:, k])
                
                U_max = np.max(ucbs, axis=1)
                chosen_arm = np.argmax(U_max)
                chosen_attr = np.argmax(ucbs[chosen_arm])  # 选中的先验索引 (0-62)
                ...
                selected_titles.append({
                    ...
                    'chosen_attr': prior_types[chosen_attr],  # 记录 pred_1 到 pred_63
...
def run_simulation_for_method(data_path, output_csv, T_list, fixed_n_llm=200, best_z_ucb=0.08):
    try:
        ...
        if results:
            ...
            agg_csv = os.path.join(csv_dir, f"MA_UCB_aggregated_attrs11.csv")
...
def main():
    try:
        ...
        output_csv = './UCB_results/MA_UCB_simulation_results.csv'
