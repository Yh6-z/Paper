...
Combos = "five" 
 # 手动修改为 two, three, four, five, six

# 自定义数据集类
class CTRDataset(Dataset):
    def __init__(self, df, tokenizer, combo_col, max_length=128):
        self.df = df.reset_index(drop=True)
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.headlines = df['headline'].astype(str).tolist()
        self.combo_labels = df[combo_col].astype(str).tolist()
    ...
    def __getitem__(self, idx):
        ...
        combo_label = self.combo_labels[idx]
        ctr = self.ctrs[idx]
        ...
        return {
            ...
            'input_ids_combo': encoding_combo['input_ids'].squeeze(),
            'attention_mask_combo': encoding_combo['attention_mask'].squeeze(),
...
class BertCrossAttentionRegressor(nn.Module):
    def __init__(self, model_name="bert-base-uncased", hidden_size=768, num_attention_heads=12, dropout_prob=0.1):
        ...
        self.cross_attention = nn.MultiheadAttention(hidden_size, num_attention_heads, dropout=dropout_prob)
        self.norm = nn.LayerNorm(hidden_size)
        self.dropout = nn.Dropout(dropout_prob)
        self.regressor = nn.Linear(hidden_size, 1)
    
    def forward(self, input_ids_headline, attention_mask_headline, 
                input_ids_combo, attention_mask_combo, labels=None):
        ...
        outputs_combo = self.bert(input_ids_combo, attention_mask=attention_mask_combo)
        hidden_combo = outputs_combo.last_hidden_state
        
        hidden_headline = hidden_headline.transpose(0, 1)
        hidden_combo = hidden_combo.transpose(0, 1)
        
        cross_attn_output, _ = self.cross_attention(hidden_headline, hidden_combo, hidden_combo)
        cross_attn_output = cross_attn_output.transpose(0, 1)
        
        attn_output = self.norm(cross_attn_output + hidden_headline.transpose(0, 1))
...
def train_model(model, train_loader, train_df, test_loader, test_df, optimizer, device, epochs=3, model_save_path="best_model.pth"):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{epochs}") as pbar:
            for batch in pbar:
                ...
                input_ids_combo = batch['input_ids_combo'].to(device)
                attention_mask_combo = batch['attention_mask_combo'].to(device)
                labels = batch['ctr'].to(device)
                
                outputs = model(
                    input_ids_headline, attention_mask_headline,
                    input_ids_combo, attention_mask_combo,
                    labels
                )
                loss = outputs['loss']
                loss.backward()
    ...
    return best_test_acc, best_train_df, best_test_df
...
def main():
    ...
    train_path = os.path.join(parent_dir, "Cross Attention","Data process","output_data", f"Train_text_sum_{Combos}_combinations.csv")
    test_path = os.path.join(parent_dir, "Cross Attention","Data process","output_data", f"Test_text_sum_{Combos}_combinations.csv")
    print("具体数据集的目录位置",train_path)
    
    # 输出路径
    output_dir = os.path.join(os.path.dirname(script_dir), f"Combo_{Combos}_RESULT")
    os.makedirs(output_dir, exist_ok=True)
    print("每个组合情况的结果的输出目录:", output_dir)
    
    accuracy_save_path = os.path.join(output_dir, f"accuracy_results_{Combos}_combinations.csv")
   ...
   # 使用循环+判断
    combo_cols = [col for col in train_df.columns if '_' in col and col not in ['test_id', 'headline', 'CTR', "sentiment",
                                                                                "length",'Concreteness_Score', 'Numeric_Structure_Score', 
                                                                                'Suspense_Question_Score', 'Pronoun_Usage','Concreteness_Label', 
                                                                                'Numeric_Label', 'Suspense_Label', 'Pronoun_Label', "Sensory_Perception_Score"]]
    print("我们使用了哪些列数据, 总共多少个:", combo_cols, len(combo_cols))
    ...
    for combo_col in combo_cols:
        if combo_col not in train_df.columns or combo_col not in test_df.columns:
            print(f"错误：{combo_col} 列不存在，跳过")
            continue
        
        print(f"\n处理组合列：{combo_col}")
        
        # 创建数据集
        train_dataset = CTRDataset(train_df, tokenizer, combo_col)
        test_dataset = CTRDataset(test_df, tokenizer, combo_col)
        ...
        # 训练模型
        model_save_path = os.path.join(output_dir, f"best_model_{combo_col}.pth")
        best_test_acc, train_df_temp, test_df_temp = train_model(
            model, train_loader, train_df, test_loader, test_df, optimizer, device, 
            epochs=3, model_save_path=model_save_path
        )
        
        # 保存预测结果
        train_save_path = os.path.join(output_dir, f"Train_Cro_Atten_{combo_col}_pred_seed.csv")
        test_save_path = os.path.join(output_dir, f"Test_Cro_Atten_{combo_col}_pred_seed.csv")
        train_df_temp.to_csv(train_save_path, index=False)
        test_df_temp.to_csv(test_save_path, index=False)
    ...
    print(f"准确率结果已保存到 {accuracy_save_path}")
